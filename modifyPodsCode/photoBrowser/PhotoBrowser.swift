...
public class PhotoBrowser: UIViewController {
    ...
    /// 转场协调器
     weak var animatorCoordinator: ScaleAnimatorCoordinator?
    
    /// presentation转场动画
     weak var presentationAnimator: ScaleAnimator?
    ...
    /// 容器layout
     let flowLayout: PhotoBrowserLayout
    
    /// PageControl
     lazy var pageControl: UIView? = { [unowned self] in
        return self.pageControlDelegate?.pageControlOfPhotoBrowser(self)
    }()
    
    /// 标记第一次viewDidAppeared
     var onceViewDidAppeared = false
    
    /// 保存原windowLevel
     var originWindowLevel: UIWindowLevel!
    
    /// 是否已初始化视图
     var didInitializedLayout = false
    
    // MARK: - 公开方法
    /// 初始化，传入用于present出本VC的VC，以及实现了PhotoBrowserDelegate协议的对象
     init(showByViewController presentingVC: UIViewController, delegate: PhotoBrowserDelegate) {
    ...
    func coverStatusBar(_ cover: Bool) {
